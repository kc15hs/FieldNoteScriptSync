...
function render(){ const event=state.entries[state.current]; if(!event)return; const source=event.source || '', reading=event.reading || '';
  ...
  bindSegments(paired); $('source-title').onclick=()=>setPlaybackStart(0); $('reading-event-number').onclick=()=>setPlaybackStart(0); updateDirty();
  requestAnimationFrame(alignSourceParagraphs);
  if(state.speaking){ if(state.sentence===0){ $('source-title').classList.add('playback-title-active'); $('reading-title').classList.add('playback-title-active'); } else highlight('reading',state.sentence-1,paired); }
}
...
function bindSegments(paired){ document.querySelectorAll('.segment').forEach(el=>el.addEventListener('click',()=>{const side=el.dataset.side,index=Number(el.dataset.index);highlight(side,index,paired);const readingIndex=side==='reading'?index:paired.findIndex(x=>x===index);if(readingIndex>=0)setPlaybackStart(readingIndex+1);})); }
function highlight(side,index,paired){ document.querySelectorAll('.segment.active').forEach(x=>x.classList.remove('active')); const other=side==='reading'?paired[index]:paired.findIndex(x=>x===index); document.querySelectorAll(`.segment[data-side="${side}"][data-index="${index}"]`).forEach(x=>x.classList.add('active')); if(other>=0) document.querySelectorAll(`.segment[data-side="${side==='reading'?'source':'reading'}"][data-index="${other}"]`).forEach(x=>x.classList.add('active')); }
